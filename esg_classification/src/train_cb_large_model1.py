...
class Model1(nn.Module):
  def __init__(self,checkpoint,num_labels): 
    ...
    self.fc6 = nn.Linear(32,num_labels)
  ...
  def forward(self, input_ids=None, attention_mask=None,labels=None):
    ...
    y = self.fc1(sequence_output[:,0,:].view(-1,1024))
    y = F.relu(y)
    y = self.fc2(y)
    y = F.relu(y)
    y = self.fc3(y)
...
def train_model(model, trainloader, valoader , num_epochs=2, accumulate_grad_batches=1):
    ...
    num_training_steps = num_epochs * (len(trainloader) // accumulate_grad_batches)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for step, batch in enumerate(tqdm(trainloader, desc = f"training")):
...
if __name__ == "__main__":
    ...
    print("\n loading model...\n")
    model=Model1(checkpoint=CHECKPOINT,num_labels=NUM_LABELS).to(device)
    ...
    for _ in tqdm(range(ROUNDS)):
        _,_,epoch_losses =  train_model(model, trainloader, valoader, num_epochs=NUM_EPOCHS, accumulate_grad_batches=4)
